...
def calculate_demographic_data(print_data=True):
    ...
    country_stats = df.groupby('native-country').agg({
        'salary': lambda x: (x == '>50K').sum() / len(x) * 100
    })
    highest_earning_country = country_stats['salary'].idxmax()
    highest_earning_country_percentage = round(country_stats['salary'].max(), 1)

    # Identify the most popular occupation for those who earn >50K in India.
    india_rich = df[(df['native-country'] == 'India') & (df['salary'] == '>50K')]
    top_IN_occupation = india_rich['occupation'].value_counts().index[0] if len(india_rich) > 0 else None
